...
describe('buildKidsWalletCards', () => {
  test('no diaper, no leap → goals, health, reminders, ask_grandma, rewards', () => {
    const ids = buildKidsWalletCards({ hasDiaper: false, hasGrowthLeap: false }).map((c) => c.id)
    expect(ids).toEqual(['goals', 'health', 'reminders', 'ask_grandma', 'rewards'])
  })

  test('diaper + leap appear in order after health', () => {
    const ids = buildKidsWalletCards({ hasDiaper: true, hasGrowthLeap: true }).map((c) => c.id)
    expect(ids).toEqual(['goals', 'health', 'diaper', 'growth_leap', 'reminders', 'ask_grandma', 'rewards'])
  })
