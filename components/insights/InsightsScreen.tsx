...
import { useTheme, brand, stickers as stickerPalette, getModeColor, getModeColorSoft, font, useDiffuseTheme, diffuseFont } from '../../constants/theme'
...
// Diffuse-aware theme swap. Under the Diffuse variant (now the default), source
// the canvas / surface / text palette + serif from the v3 tokens so this screen
// reads legibly on the Diffuse paper. Cream path is untouched (falls back to the
// current tokens when !diffuse), so existing users see no change.
//
// `ink` replaces the hardcoded STICKER_INK dark-ink text color: on the Diffuse
// canvas it routes to the v3 ink so full-strength label text stays legible; on
// cream it stays #141313 exactly as before. Soft sticker tints are kept in both
// variants (they read fine on either canvas) so we don't touch tint backgrounds.
function useInsightsTheme() {
  ...
  const colors = diffuse
    ...
    : theme.colors
  const insightsFont = diffuse
    ? { ...theme.font, display: diffuseFont.display, body: diffuseFont.body, italic: diffuseFont.italic }
    : theme.font
  const ink = diffuse ? dt.colors.ink : STICKER_INK
  return { ...theme, colors, font: insightsFont, diffuse, ink }
}
    ...
    <View style={[ci.card, { backgroundColor: colors.surface }]}>
      <Pressable onPress={() => onToggle(id)} style={ci.cardHeader}>
        ...
        <View style={[ci.cardChevronPill, { backgroundColor: colors.surfaceRaised }]}>
          <Text style={[ci.cardChevron, { color: ink + 'A0' }]}>{isOpen ? '▲' : '▼'}</Text>
        </View>
      </Pressable>
      {isOpen && (
        <View style={[ci.cardBody, { borderTopColor: ink + '12' }]}>
          {children}
          <Pressable
...
function PregnancyInsightsContent() {
    ...
    <>
      <View style={[ci.greetingCard, { backgroundColor: colors.accentSoft }]}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
          <Text style={[ci.greetingName, { color: ink }]}>{t('insights_greetingMorning', { name: parentName })}</Text>
        </View>
    ...
    <>
      <View style={[ci.warningCard, { backgroundColor: stickerPalette.peachSoft }]}>
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
          ...
          <Text style={[ci.warningTitle, { color: ink }]}>{t('insights_callProviderIf')}</Text>
        </View>
        {WARNING_SIGNS.map((sign, i) => (
          <Text key={i} style={[ci.warningItem, { color: ink + 'C8' }]}>{'• '}{sign}</Text>
        ))}
      </View>
    ...
    <>
      {featuredRead && (
        <View style={[ci.featuredCard, { backgroundColor: stickerPalette.yellowSoft }]}>
          <Text style={[ci.featuredBadge, { color: ink }]}>{t('insights_featuredThisWeek')}</Text>
          <Text style={[ci.featuredTitle, { color: ink }]}>{featuredRead.title}</Text>
          <Text style={[ci.featuredSummary, { color: ink + 'C8' }]}>{featuredRead.teaser}</Text>
          <Text style={[ci.featuredMins, { color: ink + 'A0' }]}>{t('insights_reads_minRead', { n: featuredRead.readMinutes })}</Text>
        </View>
      )}
      {allReads.map((read) => (
        <CollapsibleCard
  ...
  return (
    <View style={[ci.root, { backgroundColor: colors.bg }]}>
            ...
            <Pressable
              ...
              style={({ pressed }) => [
                ci.tabPill,
                {
                  ...
                  borderColor: active ? ink : colors.borderStrong,
                  transform: [{ translateY: active && pressed ? 2 : 0 }],
                  ...(active ? stickerShadowSm : { shadowOpacity: 0, elevation: 0 }),
                },
              ]}
            >
              <Text style={[ci.tabLabel, { color: active ? ink : colors.textMuted }]}>
                {label}
              </Text>
            </Pressable>
          )
      ...
      >
        <TalkMaster size={28} />
        <Text style={[ci.askBarText, { color: ink }]}>{t('insights_askGrandma')}</Text>
...
const ci = StyleSheet.create({
  ...
  tabLabel: { fontSize: 12, fontFamily: font.bodyBold, textTransform: 'uppercase', letterSpacing: 0.6 },
  scroll: { padding: 16, gap: 0 },

  greetingCard: { borderRadius: 26, padding: 22, marginBottom: 14, borderWidth: 1.5, borderColor: STICKER_INK + '18', ...stickerShadow },
  greetingDate: { fontSize: 11, fontFamily: font.bodySemiBold, textTransform: 'uppercase', letterSpacing: 1.4, marginBottom: 6 },
  greetingName: { fontSize: 24, fontFamily: font.display, marginBottom: 6, letterSpacing: -0.5, lineHeight: 28 },
  greetingWeek: { fontSize: 14, fontFamily: font.bodyBold },
  ...
  cardTitle: { fontSize: 16, fontFamily: font.display, letterSpacing: -0.2 },
  cardChevronPill: { width: 28, height: 28, borderRadius: 14, alignItems: 'center', justifyContent: 'center' },
  cardChevron: { fontSize: 11, fontFamily: font.bodyBold },
  cardBody: { padding: 16, paddingTop: 14, borderTopWidth: 1, gap: 10 },

  bodyText: { fontSize: 14, fontFamily: font.bodyMedium, lineHeight: 21 },
  bulletText: { fontSize: 13, fontFamily: font.bodyMedium, lineHeight: 20, paddingLeft: 4 },
  affirmationText: { fontSize: 17, fontFamily: font.italic, lineHeight: 24 },
  ...
  askCtaText: { fontSize: 13, fontFamily: font.bodyBold },

  warningCard: { borderRadius: 22, padding: 18, marginBottom: 14, borderWidth: 1.5, borderColor: STICKER_INK + '22', ...stickerShadowSm },
  warningTitle: { fontSize: 14, fontFamily: font.bodyBold, marginBottom: 10 },
  warningItem: { fontSize: 13, fontFamily: font.bodyMedium, lineHeight: 20 },

  featuredCard: { borderRadius: 26, padding: 22, marginBottom: 14, borderWidth: 1.5, borderColor: STICKER_INK + '22', ...stickerShadow },
  featuredBadge: { fontSize: 10, fontFamily: font.bodyBold, letterSpacing: 1.6, marginBottom: 10, textTransform: 'uppercase' },
  featuredTitle: { fontSize: 19, fontFamily: font.display, marginBottom: 8, letterSpacing: -0.4, lineHeight: 24 },
  featuredSummary: { fontSize: 14, fontFamily: font.bodyMedium, lineHeight: 21 },
  featuredMins: { fontSize: 12, fontFamily: font.bodySemiBold, marginTop: 8 },
  readMins: { fontSize: 11, fontFamily: font.bodySemiBold, marginTop: 6 },
  ...
  askBarText: { flex: 1, fontSize: 16, fontFamily: font.bodyBold },
})

// ─── Main Screen ──────────────────────────────────────────────────────────

export function InsightsScreen() {
  const { colors, radius, isDark, ink } = useInsightsTheme()
  const { t } = useTranslation()
  const insets = useSafeAreaInsets()
            ...
            return (
              ...
              >
                ...
                {tabKey === 'today' && insights.length > 0 && (
                  <View style={[s.tabBadge, { backgroundColor: active ? modeAccent : colors.surfaceRaised, borderColor: active ? ink : colors.border }]}>
                    <Text style={[s.tabBadgeText, { color: active ? ink : colors.textMuted }]}>
                      {insights.length}
                    </Text>
                  </View>
                )}
          ...
          <>
                ...
                <View style={s.tipBoxHeader}>
                  ...
                  {child?.birthDate && (
                    <View style={[s.agePill, { backgroundColor: stickerPalette.yellowSoft }]}>
                      <Text style={[s.agePillText, { color: ink }]}>
                  ...
                  {metrics.recentActivity.map((day, i) => {
                    ...
                    return (
                      <View key={day.date} style={s.miniBarCol}>
                        <View style={[s.miniBar, { height: h, backgroundColor: barColor, borderRadius: 6, borderColor: day.count > 0 ? ink + '40' : 'transparent' }]} />
                    ...
                    {metrics.topTypes.slice(0, 4).map(t => {
                      ...
                      return (
                        <View key={t.type} style={[s.topTypeChip, { backgroundColor: palette[metrics.topTypes.indexOf(t) % palette.length] }]}>
                          <Text style={[s.topTypeText, { color: ink }]}>{formatLogType(t.type)}</Text>
                          <Text style={[s.topTypeCount, { color: ink }]}>{t.count}</Text>
                        </View>
                      )
                    })}
                  </View>
            ...
            {generating && (
              ...
              >
                ...
                <View style={{ flex: 1 }}>
                  <Text style={[s.generatingTitle, { color: ink }]}>{t('insights_grandmaThinking')}</Text>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} style={s.categoryScroll} contentContainerStyle={s.categoryContent}>
              <Pressable
                ...
                style={({ pressed }) => [
                  s.categoryPill,
                  {
                    ...
                    borderColor: articleCategory === 'all' ? ink : colors.borderStrong,
                    transform: [{ translateY: articleCategory === 'all' && pressed ? 2 : 0 }],
                    ...(articleCategory === 'all' ? stickerShadowSm : { shadowOpacity: 0, elevation: 0 }),
                  },
                ]}
              >
                <Text style={[s.categoryPillText, { color: articleCategory === 'all' ? ink : colors.textSecondary }]}>{t('insights_reads_all')}</Text>
              </Pressable>
              {(Object.keys(CATEGORY_META) as ArticleCategory[]).map((cat) => {
                ...
                return (
                  ...
                  >
                    <Text style={[s.categoryPillText, { color: active ? ink : colors.textSecondary }]}>{meta.label}</Text>
                  </Pressable>
                )
              })}
            </ScrollView>
        ...
        <View style={s.modalSheet}>
          ...
          >
            <View style={s.modalHandle}>
              <View style={[s.handleBar, { backgroundColor: ink + '40' }]} />
            </View>
            <Pressable
              onPress={onClose}
              hitSlop={12}
...
function InsightCard({
  ...
  return (
    ...
    >
      <View style={[s.card, { backgroundColor: config.gradient[0], borderColor: ink, ...stickerShadow }]}>
        <View style={[s.cardRail, { backgroundColor: config.color, borderRightColor: ink }]} />
        <View style={s.cardInner}>
          <View style={s.cardTop}>
            ...
            >
              <X size={14} color={ink} strokeWidth={2.75} />
            </Pressable>
          </View>
          <Text style={[s.cardTitle, { color: ink }]} numberOfLines={2}>{insight.title}</Text>
          <Text style={[s.cardBody, { color: ink + 'CC' }]} numberOfLines={3}>{insight.body}</Text>
          <View style={[s.cardFooter, { borderTopColor: ink + '14' }]}>
            <View style={[s.modeBadge, { backgroundColor: paper, borderColor: ink + '30' }]}>
              <Text style={[s.modeText, { color: ink }]}>{insight.behavior}</Text>
            </View>
            <View style={[s.tapHint, { backgroundColor: paper, borderColor: ink + '24' }]}>
              <Text style={[s.tapHintText, { color: ink }]}>{t('insights_tapForDetails')}</Text>
              <ChevronRight size={12} color={ink} strokeWidth={2.75} />
            </View>
          </View>
        </View>
      </View>
          ...
          >
            ...
            <View style={[s.tipCard, { backgroundColor: config.gradient[0], borderColor: ink }]}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={[s.tipText, { color: ink + 'CC' }]}>{config.tip}</Text>
              </View>
            </View>
            <View style={s.modalMeta}>
              <View style={[s.modalMetaPill, { borderColor: ink + '24' }]}>
                <Text style={[s.modalMetaPillText, { color: ink }]}>{t('insights_modeLabel', { mode: insight.behavior })}</Text>
              </View>
              <Text style={[s.modalDateText, { color: ink + 'A0' }]}>
...
export function InsightCardCompact({ insight }: { insight: Insight }) {
  const { colors, radius } = useInsightsTheme()
  const config = TYPE_CONFIG[insight.type as InsightType] ?? TYPE_CONFIG.nudge
  const Icon = config.icon
...
const s = StyleSheet.create({
  ...
  heading: { fontSize: 38, fontFamily: font.display, letterSpacing: -1.2, marginBottom: 6, lineHeight: 42 },
  subtitle: { fontSize: 14, fontFamily: font.bodyMedium, lineHeight: 20, marginBottom: 22 },
  ...
  tabText: { fontSize: 13, fontFamily: font.bodySemiBold, letterSpacing: 0.1 },
  ...
  tabBadgeText: { fontSize: 10, fontFamily: font.bodyBold },
  ...
  dailyGreeting: { fontSize: 11, fontFamily: font.bodySemiBold, textTransform: 'uppercase', letterSpacing: 1.4, marginBottom: 4 },
  dailyName: { fontSize: 26, fontFamily: font.display, letterSpacing: -0.6, lineHeight: 30 },
  ...
  tipBoxLabel: { fontSize: 10, fontFamily: font.bodyBold, textTransform: 'uppercase', letterSpacing: 1.4, flex: 1 },
  agePill: { paddingHorizontal: 9, paddingVertical: 2, borderRadius: 999, borderWidth: 1, borderColor: STICKER_INK + '30' },
  agePillText: { fontSize: 10, fontFamily: font.bodyBold },
  tipBoxText: { fontSize: 14, fontFamily: font.bodyMedium, lineHeight: 22 },
  ...
  weekTitle: { fontSize: 18, fontFamily: font.display, flex: 1, letterSpacing: -0.3 },
  ...
  streakText: { fontSize: 11, fontFamily: font.bodyBold },
  weekNarrative: { fontSize: 13, fontFamily: font.bodyMedium, lineHeight: 20, marginBottom: 16 },
  ...
  miniBarLabel: { fontSize: 10, fontFamily: font.bodySemiBold, letterSpacing: 0.4 },
  ...
  topTypeText: { fontSize: 11, fontFamily: font.bodySemiBold },
  topTypeCount: { fontSize: 11, fontFamily: font.bodyBold },
  ...
  generatingTitle: { fontSize: 14, fontFamily: font.bodyBold },
  generatingSubtext: { fontSize: 12, fontFamily: font.bodyMedium, marginTop: 1 },
  ...
  errorText: { fontSize: 13, fontFamily: font.bodyMedium, flex: 1 },
  ...
  errorRetryText: { fontSize: 11, fontFamily: font.bodyBold, color: brand.error },
  ...
  sectionLabel: { fontSize: 11, fontFamily: font.bodyBold, textTransform: 'uppercase', letterSpacing: 1.4 },
  ...
  noInsightsTitle: { fontSize: 18, fontFamily: font.display, letterSpacing: -0.3 },
  noInsightsBody: { fontSize: 13, fontFamily: font.bodyMedium, textAlign: 'center', lineHeight: 20 },
  ...
  generateBtnText: { fontSize: 14, fontFamily: font.bodyBold, color: STICKER_INK },
  ...
  typeBadgeText: { fontSize: 10, fontFamily: font.bodyBold, letterSpacing: 1.2, textTransform: 'uppercase' },
  cardCloseBtn: { width: 30, height: 30, borderRadius: 15, alignItems: 'center', justifyContent: 'center', borderWidth: 1.5 },
  cardTitle: { fontSize: 18, fontFamily: font.display, marginBottom: 8, letterSpacing: -0.4, lineHeight: 24 },
  cardBody: { fontSize: 13, fontFamily: font.bodyMedium, lineHeight: 20, marginBottom: 14 },
  ...
  modeText: { fontSize: 10, fontFamily: font.bodyBold, textTransform: 'uppercase', letterSpacing: 1.2 },
  tapHint: { flexDirection: 'row', alignItems: 'center', gap: 4, paddingVertical: 4, paddingHorizontal: 10, borderRadius: 999, borderWidth: 1 },
  tapHintText: { fontSize: 11, fontFamily: font.bodySemiBold },
  ...
  quoteText: { fontSize: 16, fontFamily: font.italic, lineHeight: 24 },
  quoteAuthor: { fontSize: 12, fontFamily: font.bodySemiBold },
  ...
  grandmaCtaText: { flex: 1, fontSize: 16, fontFamily: font.bodyBold, color: STICKER_INK },

  // Reads tab
  readsIntro: { fontSize: 13, fontFamily: font.bodyMedium, lineHeight: 19, marginBottom: 14 },
  ...
  categoryPillText: { fontSize: 13, fontFamily: font.bodySemiBold },
  ...
  noArticlesText: { fontSize: 13, fontFamily: font.bodyMedium, textAlign: 'center', lineHeight: 20 },
  ...
  articleCatText: { fontSize: 10, fontFamily: font.bodyBold, textTransform: 'uppercase', letterSpacing: 1 },
  articleReadTime: { fontSize: 11, fontFamily: font.bodyMedium },
  articleTitle: { fontSize: 17, fontFamily: font.display, letterSpacing: -0.3, lineHeight: 22 },
  articleSummary: { fontSize: 13, fontFamily: font.bodyMedium, lineHeight: 19 },
  articleParagraph: { fontSize: 14, fontFamily: font.body, lineHeight: 22, marginBottom: 14 },
  ...
  historyEmptyTitle: { fontSize: 18, fontFamily: font.display, letterSpacing: -0.3 },
  historyEmptyBody: { fontSize: 13, fontFamily: font.bodyMedium, textAlign: 'center' },
  historyGroup: { marginBottom: 22 },
  historyDateLabel: { fontSize: 11, fontFamily: font.bodyBold, textTransform: 'uppercase', letterSpacing: 1.6, marginBottom: 12, marginLeft: 4 },
  ...
  historyTitle: { fontSize: 14, fontFamily: font.bodyBold },
  historyBody: { fontSize: 12, fontFamily: font.bodyMedium },
  ...
  modalTypeText: { fontSize: 11, fontFamily: font.bodyBold, textTransform: 'uppercase', letterSpacing: 1.4 },
  modalMetaPill: { paddingVertical: 5, paddingHorizontal: 11, borderRadius: 999, borderWidth: 1, backgroundColor: 'rgba(20,19,19,0.04)' },
  modalMetaPillText: { fontSize: 11, fontFamily: font.bodyBold },
  modalDateText: { fontSize: 16, fontFamily: font.italic, color: STICKER_INK + 'A0' },
  modalTitle: { fontSize: 26, fontFamily: font.display, letterSpacing: -0.7, marginBottom: 12, lineHeight: 30 },
  modalBody: { fontSize: 15, fontFamily: font.bodyMedium, lineHeight: 24, marginBottom: 22 },
  ...
  tipLabel: { fontSize: 13, fontFamily: font.bodyBold, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 4 },
  tipText: { fontSize: 14, fontFamily: font.bodyMedium, lineHeight: 21 },
  ...
  askBtnText: { fontSize: 16, fontFamily: font.bodyBold, color: STICKER_INK },
