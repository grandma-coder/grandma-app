...
> **A second visual language — v3 "Diffuse" — is being migrated in (additive, opt-in, default OFF).** See `DESIGN_SYSTEM.md §0.5`. It adds a parallel token set (`diffuseFont`, `diffuseFields`, `diffuseTypeRole`, `useDiffuseTheme()`) selected via `useThemeStore().variant` (`'current'` default | `'diffuse'`), toggled in Dev Panel → DESIGN VARIANT. **Everything above describes the current (default) system and is unchanged.** Two standing decisions during migration: (1) **stickers/icons stay active** under Diffuse — they're the icon system, not legacy; (2) the **`care` (Caregiver) *JourneyMode* is scaffold-only** — only Diffuse color tokens exist; it is NOT a `JourneyMode` value and has no mode config. Don't build Diffuse feature screens or a `care` *mode* until a later phase ships the Diffuse component primitives.

> **Caregiver *persona system* is fully built (distinct from the `care` mode above).** Not a 4th mode — a per-child role/permission overlay on the existing behavior homes. Invite → accept → scoped home, permissions enforced in UI *and* Postgres RLS, PHI-masked boot. Recent additions (see `docs/superpowers/specs/2026-07-19-caregiver-experience-design.md` + plan): per-recipient **`_shared_cards`** allowlist on `child_caregivers.permissions` (UX-only; RLS is still the boundary) resolved by `visibleCards(child, behavior)` in `lib/caregiverPermissions.ts`; the behavior homes (Kids/Pregnancy/Cycle) filter their cards through it via a `caregiverView` prop; a unified **`EssentialsWalletCard`** (pediatrician/allergies/emergency/insurance, lite vs full by the `emergency` capability) pinned on the caregiver home; decluttered caregiver tabs via `getCaregiverTabVisibility` (hides library/vault; agenda only if `log_activity`); a `ShareCardsEditor` for per-section curation; and a Dev Panel **"Simulate Caregiver"** section to walk the flow. Card vocabulary + sensitivity tiers live in `lib/caregiverCards.ts` (cycle intimate signals are OFF by default). *(Cycle-home caregiver filtering + mounting `ShareCardsEditor` into `care-circle.tsx` were pending a concurrent-branch merge at spec time — verify before relying on them.)*
